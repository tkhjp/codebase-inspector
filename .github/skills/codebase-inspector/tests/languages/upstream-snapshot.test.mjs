...
const cases = [
  ["javascript", "sample.js", "export class A { run() {} }", "class"],
  ["typescript", "sample.ts", "export class A { run(value: string): void {} }", "class"],
  ["python", "sample.py", "class A:\n    def run(self):\n        pass\n", "class"],
  ["rust", "sample.rs", "pub struct A; impl A { pub fn run(&self) {} }", "struct"],
  ["go", "sample.go", "package sample\ntype A struct{}\nfunc (a A) Run() {}\n", "struct"],
  ["java", "A.java", "public class A { public void run() {} }", "class"],
  ["kotlin", "A.kt", "class A { fun run() {} }", "class"],
  ["csharp", "A.cs", "public class A { public void Run() {} }", "class"],
  ["c", "a.c", "struct A { int value; }; int run(void) { return 0; }", "struct"],
  ["cpp", "a.cpp", "class A { public: void run() {} };", "class"],
  ["php", "A.php", "<?php class A { public function run() {} }", "class"],
  ["ruby", "a.rb", "class A\n  def run\n  end\nend\n", "class"],
  ["dart", "a.dart", "class A { void run() {} }", "class"]
];
...
describe("upstream extractor snapshot", () => {
  ...
  test.each(cases)("analyzes representative %s source", async (language, path, content, expectedKind) => {
    ...
    expect(result.types[0]?.kind).toBe(expectedKind);
